        ...
        <div class="border-2 rounded-lg p-2 w-1/4 m-auto mb-5">
            <form action="/pages/kategori" method="post">
                ...
                <label class="block text-center">
                    ...
                    <input name="kategori" type="text" class="text-center m-auto mt-1 block w-3/4 px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500" autocomplete="off" />
                    @error('kategori')
                        <p class="mt-2 text-center text-pink-600 text-sm">
                            {{ $message }}
                        </p>
                    @enderror
                    <button class="rounded-md bg-slate-500 px-3 py-2 text-white mt-2 hover:bg-slate-600 transition-all">Tambah</button>
                </label>
            </form>
        </div>
        <div class="flex flex-wrap gap-5 justify-center mb-14">
            ...
            @foreach ($kategoris as $kategori)
                ...
                <div class="w-full max-w-full px-3 mb-6 sm:w-1/2 sm:flex-none xl:mb-0 xl:w-1/4">
                    <div class="relative flex flex-col min-w-0 break-words bg-slate-600 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                                ...
                                <div class="flex-none w-full">
                                    ...
                                    <div class="flex justify-evenly">
                                        ...
                                        <div class="flex items-center justify-center py-2">
                                            <button data-target="edit-modals-component{{ $i }}" class="px-4 py-2 bg-slate-300 text-black rounded modal-trigger">Edit</button>
                                        </div>
                                        <!-- Modals Component Start -->
                                        <div class="flex items-center justify-center py-2">
                                            <button data-target="hapus-modals-component{{ $i }}" class="px-4 py-2 bg-red-500 text-white rounded modal-trigger">Hapus</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div id="edit-modals-component{{ $i }}" class="fixed inset-0 flex items-center justify-center z-50 hidden">
                    ...
                    <div id="modalContent"
                        ...
                        <form action="/pages/kategori/{{ $kategori->    id }}" class="m-auto w-50" method="POST"
                            ...
                            {{-- Nama Produk --}}
                            <div class="mb-5">
                                <input name="kategori" type="text" class="text-center m-auto mt-1 block w-1/2 px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500" autocomplete="off" value="{{ $kategori->kategori }}" />
                                @error('kategori')
                                    <p class="mt-2 text-center text-pink-600 text-sm">
                                        {{ $message }}
                                    </p>
